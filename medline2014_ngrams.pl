...
# The outpath should point to Data_Replication-->Parsed-->NGrams
# The inpath should point to Data_Replication-->ReplicationSample-->ReplicationXML-->MEDLINE

# Set path names
$outpath="B:\\Research\\RAWDATA\\MEDLINE\\2014\\Parsed\\NGrams";
...
for ($filenum=$startfile; $filenum<=$endfile; $filenum++) {
    ...
    open (OUTFILE, ">:utf8", "$outpath\\medline14\_$filenum\_ngrams.txt") or die "Can't open subjects file: medline14\_$filenum\_ngrams.txt";
    ...
    print "Reading in file: medline14n0$filenum.xml\n";
    &importfile;
...
sub importfile {
     ...
     if ($filenum<10) { $data = XMLin("$inpath\\medline14n000$filenum.xml", ForceArray => 1);}
     if($filenum>=10 && $filenum<=99) { $data = XMLin("$inpath\\medline14n00$filenum.xml", ForceArray => 1); }
     if($filenum>=100 && $filenum<=746) { $data = XMLin("$inpath\\medline14n0$filenum.xml", ForceArray => 1); }
}
                    ...
                    $translated="na"; $inprocess="na";
                           ...
                           if ($abstracttextsize>0) {
                              ...
                              foreach $l (@abstracttext) {
                                       ...
                                       # If the abstract is unstructered (i.e. "Normal", it should have no Label or NlmCategory Attribute)
